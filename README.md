...
The current platform includes 19 AI environments, 19 specialist agents, 23 reusable skills, 4 automation hooks, 21 prompt packs, 10 runtime templates, 15 curated capability packs, and policy-gated task recipes. Its ten-phase workflow runs bounded specialist lanes in parallel where the AI environment supports subagents, while preserving one decision owner and safe sequential fallbacks elsewhere. It supports npm, pnpm, Yarn, and Bun, with versioned JSON output for automation.
